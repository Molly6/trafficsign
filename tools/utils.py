...
from tools.nms import py_cpu_nms, topk_voting
    ...
    def predict(self, **inputs):
        ...
        for c in range(self.model.cfg.num_classes):
            ...
            if dtboxes.size > 0:
                ...
                nms_dets = dtboxes[keep, :]
                dtboxes = topk_voting(nms_dets, dtboxes, vote_thresh=0.85, k=5)
                dtboxes = np.hstack(
                    (dtboxes, np.full((len(dtboxes), 1), c, dtype=np.float32))
                ).astype(np.float32)
