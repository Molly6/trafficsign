...
class PointHead(M.Module):
    ...
    def __init__(self, cfg, input_shape: List[layers.ShapeSpec]):
        ...
        # )
        # self.align_conv = M.DeformableConv2d(
        #         in_channels,
        #         in_channels,
        ...
        for modules in [
            ...
            self.ctrness #,self.offset_conv, self.align_conv
        ]:
    ...
    def forward(self, features: List[Tensor]):
        ...
        for feature, scale, stride in zip(features, self.scale_list, self.stride_list):
            ...
            # offset_mask = self.offset_conv(bbox_subnet)
            # offset = offset_mask[:, :18, :, :]
            # mask = F.sigmoid(offset_mask[:, -9:, :, :])
            # cls_subnet = self.align_conv(cls_subnet, offset, mask)
